...
export class AuthService implements OnModuleInit {
    ...
    const presetUsers = [
      {
        ...
        role: 'user',
        department: '数据基础设施部',
        avatar: buildUserAvatarUrl({ employeeId: '7462201' }),
        points: 10000,
      },
      {
        name: '张伟 (SRE专家)',
        ...
        role: 'user',
        department: '运维保障中心',
        avatar: buildUserAvatarUrl({ employeeId: '7462202' }),
        points: 10000,
      },
    ];
  ...
  private async reconcileAccounts(): Promise<void> {
    ...
    // 5. 头像重写到当前 AVATAR_BASE_URL
    //    覆盖两类数据：早期硬编码的 Unsplash 外链、以及上一次部署用别的 host 生成的
    //    DiceBear 地址（外网 api.dicebear.com → 内网 10.9.43.61 的切换就靠这一步）。
    //    用户自己上传/手填的头像不属于「自动生成」，会被 isAutoGeneratedAvatar 排除。
    const allUsers = await this.userRepository.find();
    let avatarFixed = 0;
    for (const user of allUsers) {
      const next = reconcileAvatarUrl(user.avatar, user);
      if (next) {
        user.avatar = next;
        await this.userRepository.save(user);
        avatarFixed += 1;
    ...
    if (avatarFixed) {
      console.log(`✅ 已将 ${avatarFixed} 个账号头像重写到当前头像服务`);
    }

    // 6. 同步各业务表里的头像「快照」字段
    await this.reconcileAvatarSnapshots();
  }

  /**
   * 同步业务表中的头像快照字段
   *
   * skills / skill_demands / feedback 三张表都把作者头像**冗余存了一份**
   * （列表接口不必再回表查 users）。用户头像重写后这些快照不会自动更新，
   * 于是集市列表还是旧头像、个人中心已是新头像，同一个人两副面孔。
   *
   * 这里按各自的 user 外键 JOIN 回 users 表取权威头像，只覆盖「系统自动生成」
   * 的快照（旧 Unsplash 外链 / 其它 host 的 DiceBear 地址 / 空值），
   * 用户自传头像不动 —— 判定条件与 isAutoGeneratedAvatar 保持一致。
   */
  private async reconcileAvatarSnapshots(): Promise<void> {
    // 与 isAutoGeneratedAvatar 等价的 SQL 条件：空 / 旧 Unsplash / 任意 host 的 DiceBear
    const autoGenerated = `(
      $COL IS NULL OR $COL = ''
      OR $COL LIKE 'https://images.unsplash.com/%'
      OR $COL ~ '/[a-zA-Z0-9-]+/svg\\?seed='
    )`;

    const targets = [
      { table: 'skills', column: 'avatar', joinKey: 'submitter_id' },
      { table: 'skill_demands', column: 'author_avatar', joinKey: 'author_id' },
      { table: 'feedback', column: 'submitter_avatar', joinKey: 'submitter_id' },
    ];

    let total = 0;
    for (const { table, column, joinKey } of targets) {
      const result = await this.dataSource.query(
        `UPDATE ${table} AS t
            SET ${column} = u.avatar_url
           FROM users AS u
          WHERE t.${joinKey} = u.id::text
            AND u.avatar_url IS NOT NULL
            AND t.${column} IS DISTINCT FROM u.avatar_url
            AND ${autoGenerated.replace(/\$COL/g, `t.${column}`)}`,
      );
      // pg 驱动对 UPDATE 返回 [rows, affectedCount]
      const affected = Array.isArray(result) ? (result[1] ?? 0) : 0;
      total += Number(affected) || 0;
    }

    if (total) {
      console.log(`✅ 已同步 ${total} 条业务记录的头像快照`);
    }

    // 第二轮：JOIN 不上 users 的记录按「展示名」派生头像
    //   - skills 的历史数据 submitter_id 为 NULL（该字段是后来才加的）
    //   - 作者账号已被删除时外键成了孤儿
    // 这类记录没有权威用户可参照，只能退而用冗余存下来的姓名做 seed。
    // 同名会得到同一头像，但优于继续留着旧的 Unsplash 外链。
    //
    // 关键：必须排除「第一轮已经关联到用户」的行（AND NOT EXISTS ...）。
    // 否则第一轮把快照同步成用户头像(seed=工号)后，第二轮又按姓名(seed=姓名)
    // 覆盖回去，两轮互相打架，每次启动都刷一遍且永远收敛不了。
    // 同时只在「算出来的值和现值不同」时才写库，保证重复启动零更新。
    const nameFallbacks = [
      { table: 'skills', column: 'avatar', nameColumn: 'author', joinKey: 'submitter_id' },
      {
        table: 'skill_demands',
    ...
    for (const { table, column, nameColumn, joinKey } of nameFallbacks) {
      const rows: Array<{ id: string; nm: string | null; cur: string | null }> =
        await this.dataSource.query(
          `SELECT t.id, t.${nameColumn} AS nm, t.${column} AS cur
             FROM ${table} AS t
            WHERE ${autoGenerated.replace(/\$COL/g, `t.${column}`)}
              AND NOT EXISTS (
                SELECT 1 FROM users AS u
                 WHERE u.id::text = t.${joinKey}
                   AND u.avatar_url IS NOT NULL
              )`,
        );
    ...
    if (fallbackFixed) {
      console.log(
        `✅ 已按作者姓名补齐 ${fallbackFixed} 条无法关联用户的头像快照`,
      );
    }
  }
  ...
  private async ensureSuperAdmin(): Promise<void> {
    ...
    const superAdmin = this.userRepository.create({
      ...
      department: '基础架构部',
      avatar: buildUserAvatarUrl({ loginName: SUPER_ADMIN_LOGIN }),
      points: 10000,
    });
  ...
  async register(dto: RegisterDto): Promise<AuthResponse> {
    ...
    const newUser = this.userRepository.create({
      ...
      role: 'user',
      menuPermissions: [],
      avatar: dto.avatar || buildUserAvatarUrl({ employeeId }),
      points: 10000,
    });
  ...
  async ossLogin(employeeId: string, clientIp?: string): Promise<AuthResponse> {
    ...
    if (!user) {
      ...
      user = this.userRepository.create({
        ...
        avatar: buildUserAvatarUrl({ employeeId: profile.employeeId }),
        points: 10000,
      });
