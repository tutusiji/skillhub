...
2. For rows that cannot be joined, derive the avatar from the denormalised display name (`author` / `author_name` / `submitter_name`). This pass is required because `skills.submitter_id` is NULL for all historical rows (the column was added later) and a couple of demands point at deleted users.
